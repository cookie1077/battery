...
from model import RNN_LSTM, RNN_GRU, TransformerModel, RNN_LU
...
def build_dataset_batched(data_x, data_y, seq_length):
    ...
    current = data_x[:, 0:2]
    qv_matrix = data_x[:seq_length, 2:]

    data_out_x = []
    data_out_y = []

    for i in range(0, len(data_x) - seq_length):
        _x = current[i:i + seq_length, :]
        _x = list(np.concatenate((np.array(_x), np.array(qv_matrix)), axis=1))
    ...
    '''
    # Reshape to 2D
    x_2d = data_out_x.reshape(-1, data_out_x.shape[-1])
    ...
    data_out_x = x_scaled.reshape(data_out_x.shape)
    data_out_x = torch.FloatTensor(data_out_x)
    data_out_x = data_out_x.to(device)
    '''
...
def run_experiment_lu(iteration, epochs=200, seq_length=10, lr=1e-3):
    ...
    for epoch in range(epochs):
        ...
        for data_name in train_list:
            ...
            train_x, train_y = build_dataset_batched(x, y, seq_length=seq_length)
        ...
        if epoch % 10 == 9:  # Evaluating the test set for every 10 epochs
            ...
            for file_name in test_list:
                ...
                test_x, test_y = build_dataset_batched(x, y, seq_length=seq_length)
                ...
                with torch.no_grad():
                    ...
                    rmse_loss_tmp = np.sqrt(mean_squared_error(y_true_tmp[seq_length:].cpu().detach().numpy(), y_pred_tmp.cpu().detach().numpy()))
                    r2_tmp = r2_score(y_true_tmp[seq_length:].cpu().detach().numpy(), y_pred_tmp.cpu().detach().numpy())
    ...
    for file_name in test_list:
        ...
        with torch.no_grad():
            ...
            rmse_loss = np.sqrt(mean_squared_error(y_true[seq_length:].cpu().detach().numpy(), y_pred.cpu().detach().numpy()))
            r2 = r2_score(y_true[seq_length:].cpu().detach().numpy(), y_pred.cpu().detach().numpy())
...
def run_experiment_gru(iteration, epochs=100, seq_length=10, lr=1e-3):
    ...
    # train : model to device! could change the dimensions.
    model = RNN_GRU(input_dim=2, hidden_dim=10, output_dim=1, layers=3).to(device)
    #print(list(model.parameters()))
    ...
    for epoch in range(epochs):
        ...
        for data_name in train_list:
            ...
            # Key : use only two 
            x = data[:, 3:]  # [current, capacity_prev]
            y = data[:, 1][:, None]  # Capacity
        ...
        if epoch % 10 == 9:  # Evaluating the test set for every 10 epochs
            ...
            for file_name in test_list:
                ...
                x = test_data[:, 3:]  # [current, capacity_prev]
                y = test_data[seq_length:, 1][:, None]  # Capacity, just use y instead of test_y
...
def run_experiment_lstm(iteration, epochs=100, seq_length=10, lr=1e-3):
    ...
    # train
    model = RNN_LSTM(input_dim=2, hidden_dim=10, output_dim=1, layers=3).to(device)
    optimizer = optim.Adam(model.parameters(), lr=lr, weight_decay=1e-6)
...
def run_experiment_transformer(iteration, epochs=200, seq_length=10, lr=1e-3):
    ...
    # train
    model = TransformerModel(input_dim=2, hidden_dim=32, output_dim=1, num_layers=3, nhead=8, dropout=0.2).to(device)
    optimizer = optim.Adam(model.parameters(), lr=lr, weight_decay=1e-6)
...
for i in range(times):
    ...
    error.append(run_experiment_gru(i, epochs = 200, seq_length=10, lr=7e-3))
    #run_experiment_lstm(i, epochs = 200, seq_length=10, lr=7e-3)
