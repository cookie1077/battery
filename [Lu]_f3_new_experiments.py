...
####################################################################################
data_path = "Code_for_intern/dataset_f3/"
...
def run_experiment_gru(iteration, epochs=100, seq_length=10, lr=1e-3):
    ...
    for epoch in range(epochs):
        ...
        for data_name in train_list:
            data = torch.load(data_new_path + data_name)
            f3_data = torch.load(data_path + data_name)

            # Key : use only two 
            x = f3_data
            #print('data is', x.shape)

            min_0, min_1 = x.min(dim=0)[0].min(dim=0)[0]
            max_0, max_1 = x.max(dim=0)[0].min(dim=0)[0]

            x[:, :, 0:1] = ((x[:, :, 0:1] - min_0) / (max_0 - min_0)) - (max_0 - min_0)/2
            x[:, :, -1:] = ((x[:, :, -1:] - min_1) / (max_1 - min_1)) - (max_1 - min_1)/2
        ...
        if epoch % 10 == 9:  # Evaluating the test set for every 10 epochs
            ...
            for file_name in test_list:
                test_data = torch.load(data_new_path + file_name)
                test_f3_data = torch.load(data_path + file_name)
...
for i in range(1):
    ...
    run_experiment_gru(i, epochs = 200, seq_length=10, lr=7e-3)
