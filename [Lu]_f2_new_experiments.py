...
####################################################################################
data_path = "Code_for_intern/dataset_f2/"
...
def run_experiment_gru(iteration, epochs=100, seq_length=10, lr=1e-3):
    ...
    # train : model to device! could change the dimensions.
    model = RNN_GRU(input_dim=4, hidden_dim=10, output_dim=1, layers=3).to(device)
    #print(list(model.parameters()))
    ...
    for file_name in test_list:
        ...
        x = test_data[:, 1:]  # [current, capacity_prev]
        y = test_data[seq_length:, 0][:, None]  # Capacity
...
def run_experiment_lstm(iteration, epochs=100, seq_length=10, lr=1e-3):
    ...
    # train
    model = RNN_LSTM(input_dim=4, hidden_dim=10, output_dim=1, layers=3).to(device)
    optimizer = optim.Adam(model.parameters(), lr=lr, weight_decay=1e-5)
    ...
    for epoch in range(epochs):
        ...
        for data_name in train_list:
            ...
            x = data[:, 1:]  # [current, capacity_prev]
            y = data[:, 0][:, None]  # Capacity
...
def run_experiment_transformer(iteration, epochs=200, seq_length=10, lr=1e-3):
    ...
    # train
    model = TransformerModel(input_dim=4, hidden_dim=32, output_dim=1, num_layers=3, nhead=8, dropout=0.2).to(device)
    optimizer = optim.Adam(model.parameters(), lr=lr, weight_decay=1e-5)
...
for i in range(1):
    ...
    run_experiment_gru(i, epochs = 400, seq_length=10, lr=6e-3)
    run_experiment_lstm(i, epochs = 400, seq_length=10, lr=1e-2)
